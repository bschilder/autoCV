...
pkg <- read.dcf(file.path("DESCRIPTION"), fields = "Package")[1]
description <- read.dcf(file.path("DESCRIPTION"), fields = "Description")[1]
```
...
```{r}
filename <- file.path("inst","hex","hex.png")
...
ggcube  <- ggplot2::ggplot(as.data.frame(obj)) + 
    ...
    #     colors = ggplot2::alpha(rev(RColorBrewer::brewer.pal(9, "viridis")), 
    #                             alpha = .5)) + 
    ggplot2::scale_fill_viridis_c(alpha = .75, option = "mako") +
    ggplot2::theme_void() + 
    ggplot2::coord_equal()
ggcube
```
...
stick <- hexSticker::sticker( 
  subplot = ggcube,
  #### Package name ####
  package = pkg, p_size=20, p_y = 1.45, p_color = ggplot2::alpha("black",.5),
  #### Subplot #####
  s_x=1, s_y=1,  s_height = s_size, s_width = s_size,
  #### Fill & border ####
  h_fill = "white", h_color = ggplot2::alpha("black",.25),
  #### Spotlight ####
  spotlight = TRUE, l_alpha = .01, l_width = 10, l_x = .5, l_y = .75,
  #### File output ####
